exports.run = async (client, msg, args) => {
  let kufur=[
    ...
    "Ananın amına yoğurt koyarım sikerken 5 dakikada yoğurt yaparım",
    "geçme namık kemal köprüsünden incitirsin vakvakları ananınamınaçamdiktim git topla kozalakları",
  ]
 ...
 description: (':no_entry_sign: Ya geçerli birini etiketle ya da sana sövmemi istiyosan kendini etiketle.')
  ...
  if(member.id === "685579696410329173")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (':no_entry_sign: Sen Kimsin Sahibime Sövüyon Amına Kodumun Jedayı.')
}})
  if(member.id === "685579696410329173")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop! Birşeyler ters gitti <@669888540032761887> etiketlemişsin yarram!`)
}})
  if(member.id === "685579696410329173")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop! Orda durucan Yapımcım <@669888540032761887>'ya sövemezsin amını sikerim.`)
}})
